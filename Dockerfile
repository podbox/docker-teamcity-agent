...
RUN apt-get update \
 && apt-get install -yqq git libfontconfig zip python-dev python-pip \
 && apt-get clean \
...
RUN (curl -L http://mirrors.ibiblio.org/apache/maven/maven-3/$MAVEN_VERSION/binaries/apache-maven-$MAVEN_VERSION-bin.tar.gz | gunzip -c | tar x) \
